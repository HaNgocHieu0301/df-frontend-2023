...
const Pagination = () => {
  ...
  const pagingElement: Array<any> = [];
  if (state.currentPage > 1) {
    pagingElement.push(
      <button
        key="prev"
        className="btn btn-xs btn-bg-blue"
        onClick={() => dispatch(actions.changeCurrentPage(state.currentPage - 1))}
      >
        Prev
      </button>,
    );
  ...
  let rightIndex =
    state.currentPage + 2 > state.totalPage ? state.totalPage : state.currentPage + 2;
  ...
  for (let index = leftIndex; index <= rightIndex; index++) {
    const customClass =
      index === state.currentPage ? 'btn btn-xs btn-bg-red' : 'btn btn-xs btn-bg-blue';
    pagingElement.push(
      <button
        key={index}
